...
// ==========================================
const SKILLS: Skill[] = [
  { title: 'Web Development', icon: '</>', tech: 'Laravel, Next.js, Tailwind, PHP', desc: 'Membangun API solid dan antarmuka web responsif.' },
...
const PROJECTS: Project[] = [
  { id: 1, title: 'Secure REST API', category: 'Web', tech: ['Laravel', 'Sanctum', 'MySQL'], desc: 'Backend operasi CRUD dengan autentikasi token untuk keamanan akses data.', link: 'https://github.com/AK-Khanif' },
  { id: 2, title: 'Alphabet Adventure', category: 'Game', tech: ['Godot', 'GDScript'], desc: 'Game edukasi 2D dengan sistem auto-restart tanpa layar Game Over.', link: 'https://github.com/AK-Khanif' },
  { id: 3, title: 'Real-Time Monitor', category: 'IoT', tech: ['NodeMCU', 'MQTT', 'DHT11'], desc: 'Sistem kendali LED & sensor suhu berbasis Web Dashboard.', link: 'https://github.com/AK-Khanif' },
  { id: 4, title: 'Native Android App', category: 'Mobile', tech: ['Kotlin', 'MVVM'], desc: 'Aplikasi mobile berstandar industri menggunakan Repository Pattern.', link: 'https://github.com/AK-Khanif' },
  { id: 5, title: 'Sistem Daftar Ulang', category: 'Web', tech: ['Laravel', 'MySQL'], desc: 'Platform administrasi akademik web untuk pendaftaran ulang mahasiswa.', link: 'https://github.com/AK-Khanif' },
  { id: 6, title: 'Sistem Reservasi Meja', category: 'Web', tech: ['PHP Native', 'MySQL'], desc: 'Aplikasi web operasional untuk meminimalkan double-booking.', link: 'https://github.com/AK-Khanif' },
];
...
const SkillCard = ({ skill }: { skill: Skill }) => (
  <div className="group p-8 rounded-3xl bg-slate-900/80 backdrop-blur-sm border border-slate-800 hover:border-cyan-500/50 hover:bg-slate-800 hover:-translate-y-2 hover:shadow-[0_10px_30px_rgba(34,211,238,0.1)] transition-all duration-300 relative overflow-hidden cursor-default">
...
const ProjectCard = ({ project }: { project: Project }) => (
  <article className="group p-8 rounded-[2rem] bg-slate-900/60 backdrop-blur-sm border border-slate-800 hover:border-cyan-500/50 transition-all duration-500 hover:-translate-y-3 hover:shadow-[0_20px_40px_-15px_rgba(34,211,238,0.2)] flex flex-col relative overflow-hidden animate-fade-in">
...
const ExperienceTimeline = ({ exp, index }: { exp: Experience, index: number }) => (
  <div className="relative pl-8 md:pl-0 flex flex-col md:flex-row items-center justify-between group">
    ...
    <div className={`w-full md:w-[45%] p-6 md:p-8 rounded-3xl bg-slate-900/60 border border-slate-800 group-hover:border-cyan-500/50 group-hover:-translate-y-2 group-hover:shadow-[0_10px_30px_rgba(34,211,238,0.1)] transition-all duration-300 ${index % 2 === 0 ? 'md:text-right md:mr-auto' : 'md:text-left md:ml-auto'}`}>
...
export default function Home() {
  ...
  return (
    <main className="min-h-screen bg-slate-950 text-slate-300 font-sans overflow-x-hidden selection:bg-cyan-500 selection:text-slate-900 scroll-smooth custom-cursor-area">
      ...
      <div ref={cursorOutlineRef} className="cursor-outline hidden md:flex justify-center items-center w-8 h-8 border border-cyan-400/50 rounded-full fixed top-0 left-0 pointer-events-none z-[100] -ml-4 -mt-4">
      ...
      <div ref={bgParallaxRef} className="fixed top-0 left-0 w-full h-full pointer-events-none z-0 opacity-40">
        <div className="absolute top-[20%] left-[10%] w-[500px] h-[500px] bg-cyan-600/20 rounded-full blur-[120px]"></div>
        <div className="absolute bottom-[10%] right-[10%] w-[600px] h-[600px] bg-blue-700/10 rounded-full blur-[150px]"></div>
      </div>
      ...
      <nav className={`fixed top-10 md:top-12 left-0 right-0 z-50 transition-all duration-300 ${isScrolled ? 'py-2' : 'py-4'}`}>
        ...
        <div className={`fixed inset-0 bg-slate-950/95 backdrop-blur-xl z-40 transition-transform duration-300 ease-in-out flex flex-col justify-center items-center gap-8 ${isMobileMenuOpen ? 'translate-y-0' : '-translate-y-full'}`}>
           {['Profil', 'Keahlian', 'Proyek', 'Pengalaman', 'Kontak'].map((item) => (
              <a key={item} href={`#${item.toLowerCase()}`} onClick={() => setIsMobileMenuOpen(false)} className="text-2xl font-bold text-white hover:text-cyan-400 transition-colors">
                {item}
              </a>
            ))}
        </div>
      ...
      {/* Hero Section */}
      <section id="profil" className="scroll-mt-32 max-w-6xl mx-auto px-6 min-h-screen flex flex-col-reverse md:flex-row justify-center items-center gap-12 pt-32 pb-20">
        <div className="flex-1 space-y-6 md:space-y-8 relative z-10 text-center md:text-left">
          <div className="inline-flex items-center justify-center md:justify-start gap-2 px-4 py-2 rounded-full bg-slate-900/80 border border-slate-700/50 backdrop-blur-sm">
          ...
          <div className="flex flex-wrap justify-center md:justify-start gap-4 pt-2">
            ...
            <a href="https://github.com/AK-Khanif" target="_blank" rel="noreferrer" className="px-8 py-3.5 rounded-full bg-slate-900 border border-slate-700 text-white font-bold hover:bg-slate-800 hover:border-slate-500 active:scale-95 transition-all duration-300 flex items-center gap-2">
              GitHub Profile
            </a>
          </div>
        </div>
      ...
      {/* Keahlian Section (DITAMBAHKAN scroll-mt-32) */}
      <section id="keahlian" className="scroll-mt-32 py-24 relative z-10 bg-slate-900/40 border-y border-slate-800/80">
      ...
      {/* Proyek Section (DITAMBAHKAN scroll-mt-32) */}
      <section id="proyek" className="scroll-mt-32 py-32 max-w-7xl mx-auto px-6 relative z-10">
        <div className="text-center mb-12">
          ...
          <div className="flex flex-wrap justify-center gap-3">
            {FILTERS.map((filter) => (
              <button key={filter} onClick={() => setActiveFilter(filter)} className={`px-6 py-2 rounded-full text-sm font-bold transition-all duration-300 active:scale-95 ${activeFilter === filter ? 'bg-cyan-500 text-slate-950 shadow-[0_5px_15px_rgba(34,211,238,0.4)]' : 'bg-slate-900/80 text-slate-400 border border-slate-700 hover:border-slate-500 hover:text-white'}`}>
                {filter}
              </button>
            ))}
          </div>
      ...
      {/* Pengalaman Section (DITAMBAHKAN scroll-mt-32) */}
      <section id="pengalaman" className="scroll-mt-32 py-32 bg-slate-900/40 border-t border-slate-800/80 relative z-10">
        <div className="max-w-4xl mx-auto px-6">
          <div className="text-center mb-16">
            ...
            <p className="text-slate-400">Terbiasa bekerja secara profesional, disiplin, dan menyelesaikan masalah di lapangan.</p>
          </div>
          <div className="relative border-l-2 border-slate-800 ml-4 md:ml-0 md:pl-0 space-y-12 before:hidden md:before:block before:absolute before:inset-0 before:mx-auto before:w-0.5 before:bg-slate-800">
            {EXPERIENCES.map((exp, idx) => (
              <ExperienceTimeline key={idx} exp={exp} index={idx} />
            ))}
          </div>
        </div>
      </section>

      {/* Kontak Section (DITAMBAHKAN scroll-mt-32) */}
      <section id="kontak" className="scroll-mt-32 py-32 relative overflow-hidden border-t border-slate-800 z-10">
        ...
        <div className="max-w-3xl mx-auto px-6 text-center">
          ...
          <p className="text-lg text-slate-400 mb-12">Saya sedang mencari pekerjaan tetap atau peluang magang di bidang IT. Mari berdiskusi tentang bagaimana saya bisa berkontribusi di tim Anda.</p>
      ...
      <footer className="py-8 text-center border-t border-slate-800/80 bg-slate-950 relative z-10">
        <p className="text-sm font-medium text-slate-500">© {new Date().getFullYear()} Awwalul Khoirul Khanif. Semarang, Indonesia.</p>
      </footer>
